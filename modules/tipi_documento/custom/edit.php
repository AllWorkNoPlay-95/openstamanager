...
$id_module_acquisti = Module::where('name', 'Fatture di acquisto')->first()->id;
$id_module_vendite = Module::where('name', 'Fatture di vendita')->first()->id;
...
echo '
		...
		<div class="col-md-3">
			{[ "type": "select", "label": "'.tr('Sede aziendale predefinita').'", "name": "mncs_id_sede_partenza", "ajax-source": "sedi_azienda", "value": "$mncs_id_sede_partenza$", "help": "'.tr('Sede impostata in automatico nei documenti di questo tipo. In vendita è la sede di partenza, in acquisto la sede di destinazione (arrivo). Vuoto = comportamento standard (sede utente).').'" ]}
		</div>
