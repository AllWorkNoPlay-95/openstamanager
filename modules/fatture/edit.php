...
if ($dir == 'entrata') {
    echo '
				<div class="col-md-3">
					{[ "type": "select", "label": "'.tr('Agente di riferimento').'", "name": "id_agente", "ajax-source": "agenti", "select-options": {"id_anagrafica": '.$record['id_anagrafica'].'}, "value": "$id_agente_fattura$" ]}
				</div>';
}
...
if ($record['descrizione_tipo'] == 'Fattura accompagnatoria di vendita') {
    ...
    <div class="card card-info">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-3">
                    {[ "type": "select", "label": "'.tr('Porto').'", "name": "id_portoo", "placeholder": "", "values": "query=SELECT `dt_porto`.`id`, `dt_porto_lang`.`title` as descrizione FROM `dt_porto` LEFT JOIN `dt_porto_lang` ON (`dt_porto`.`id` = `dt_porto_lang`.`id_record` AND `dt_porto_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).') ORDER BY `title` ASC", "value": "id_portoto$" ]}
                </div>
    ...
    $esterno = $dbo->selectOne('dt_spedizione', 'esterno', [
        'id' => $record['id_spedizione'],
                    ...
                    {[ "type": "select", "label": "<?php echo tr('Vettore'); ?>", "name": "id_vettore", "ajax-source": "vettori", "value": "$id_vettore$", "disabled": <?php echo empty($esterno) || (!empty($esterno) && !empty($record['id_vettore'])) ? 1 : 0; ?>, "required": <?php echo !empty($esterno) ?: 0; ?>, "icon-after": "add|<?php echo $id_modulo_anagrafiche; ?>|tipoanagrafica=Vettore&readonly_tipo=1|btn_id_vettore|<?php echo ($esterno and (intval(!$record['flag_completato']) || empty($record['id_vettore']))) ? '' : 'disabled'; ?>", "class": "<?php echo empty($record['id_vettore']) ? 'unblockable' : ''; ?>" ]}
                </div>
                        ...
                        <div class="col-md-3" style="margin-top: 25px">
                            ...
                            <div class="btn-group tip" data-card-widget="tooltip">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <a class="btn dropdown-item" style="cursor:pointer" onclick="gestioneDescrizione(this)" data-title="'.tr('Aggiungi descrizione').'">
...
<script type="text/javascript">
    ...
    $("#id_tipo_documento").change(function() {
         ...
         if (tipoData && tipoData.mncs_id_sede_partenza != null && tipoData.mncs_id_sede_partenza !== "") {
             $("#'.($dir == 'entrata' ? 'id_sede_partenza' : 'id_sede_destinazione').'").selectSetNew(tipoData.mncs_id_sede_partenza, tipoData.nome_sede);
         }
    });
