...
// Mini-form: metodo di pagamento + sede + importo, con esito dinamico (abbuono / fattura aperta).
...
$residuo = round(floatval($dbo->fetchOne('SELECT SUM(ABS(`da_pagare`) - ABS(`pagato`)) AS residuo FROM `co_scadenzario` WHERE `id_documento` = '.prepare($id_record))['residuo']), 2);
...
<form action="<?php echo $action; ?>" method="post" id="add-form">
	...
	<div class="callout callout-info" style="margin-bottom: 18px;">
		<div class="row">
			...
			<div class="col-xs-5 text-right">
				<div class="text-muted" style="font-size: 0.85em;"><?php echo tr('Residuo da incassare'); ?></div>
	...
	<div class="row">
		...
		<div class="col-md-4">
			{[ "type": "select", "label": "<?php echo tr('Sede'); ?>", "name": "id_sede", "required": 1, "ajax-source": "sedi_azienda", "value": "<?php echo intval($fattura->id_sede_partenza); ?>" ]}
		</div>
