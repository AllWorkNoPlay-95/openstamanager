...
DELETE FROM `zz_settings_lang` WHERE `id_record` = @mncs_set;
INSERT INTO `zz_settings_lang` (`id_lang`, `id_record`, `title`, `help`) VALUES
  (1, @mncs_set, 'GECOM mappa IVA', 'JSON {"id aliquota OSM": "codice GECOM a 3 cifre"}. Es. {"161": "022", "163": "004"}. Per le aliquote ordinarie il codice è la percentuale (022, 010, 004); per esenzioni/non imponibili usare il codice GECOM dedicato (es. 315, 374). Le aliquote usate nel periodo ma non mappate bloccano l''export.'),
...
DELETE FROM `zz_settings_lang` WHERE `id_record` = @mncs_set;
INSERT INTO `zz_settings_lang` (`id_lang`, `id_record`, `title`, `help`) VALUES
  (1, @mncs_set, 'GECOM mappa conti', 'JSON {"id conto OSM (co_piano_dei_conti3)": "conto GECOM a 7 cifre"}. Es. {"34": "5805010"}. I conti non mappati usano "GECOM conto ricavo".'),
...
DELETE FROM `zz_settings_lang` WHERE `id_record` = @mncs_set;
INSERT INTO `zz_settings_lang` (`id_lang`, `id_record`, `title`, `help`) VALUES
  (1, @mncs_set, 'GECOM mappa documenti', 'JSON {"id tipo documento OSM": {"causale": "001", "descrizione": "Fattura di vend", "sezionale": "1", "nota_credito": false}}. Solo i tipi presenti nella mappa vengono esportati. Causali osservate: 001 fattura di vendita, 002 nota credito cliente. Sezionali: 1 = FE (Feroleto), 0 = RE (Rende), 6 = NF, 5 = NR.'),
  (2, @mncs_set, 'GECOM documents map', 'JSON {"OSM document type id": {"causale", "descrizione", "sezionale", "nota_credito"}}. Only mapped types are exported.');
